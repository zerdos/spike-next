...
Local secrets go in `.dev.vars` (gitignored): `GEMINI_API_KEY`, `RESEND_API_KEY`,
`SESSION_SIGNING_SECRET`, `PII_ENCRYPTION_KEY` (base64 AES-256 key). In production these are
set with `wrangler secret put <NAME>`.

## Cost guardrails

The agent chat calls the Gemini API on every turn. Three layers keep spend bounded:
...
3. **Org-level spend alerts**: set a monthly spend cap and budget alerts in Google AI Studio /
   Google Cloud billing (verify the current product surface at setup time). This is the backstop
   layer and must be configured manually (no API for it) before launch.

`AGENT_MODEL` (default `gemini-2.5-flash`) is a single env var — switching to a different model
tier (e.g. `gemini-2.5-pro`) needs no code change, only a redeploy, and should be re-validated
